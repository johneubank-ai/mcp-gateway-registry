...
const StarRatingWidget: React.FC<StarRatingWidgetProps> = ({
  ...
  const renderStars = (count: number, filled: boolean, size: 'small' | 'large' = 'large') => {
    ...
    return (
      <Star
        className={`${sizeClass} ${filled ? 'text-amber-400 fill-amber-400' : 'text-muted-foreground'}`}
      />
    );
  };
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      <button
        ...
        className="flex items-center gap-2 hover:bg-accent p-2 rounded-lg transition-colors duration-200"
      ...
      {isDropdownOpen && (
        <div
          className="absolute top-full left-0 mt-2 w-80 bg-card rounded-lg shadow-xl border border-border z-50 p-4"
          role="dialog"
              ...
              >
                {[1, 2, 3, 4, 5].map((star) => (
                  <button
                    ...
                    className="p-1 hover:scale-110 transition-transform duration-150 focus:outline-hidden focus:ring-2 focus:ring-ring rounded"
                    role="radio"
